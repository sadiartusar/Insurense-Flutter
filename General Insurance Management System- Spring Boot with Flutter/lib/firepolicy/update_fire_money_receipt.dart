...
class _UpdateFireMoneyReceiptState extends State<UpdateFireMoneyReceipt> {
  ...
  void _populateInitialData() {
    ...
    selectedPolicyholder = widget.moneyreceipt.fireBill?.firePolicy.policyholder ?? '';
    selectedBankName = widget.moneyreceipt.fireBill?.firePolicy.bankName ?? '';
    selectedSumInsured = widget.moneyreceipt.fireBill?.firePolicy.sumInsured ?? 0.0;
  }
  ...
  Widget _buildDropdownField() {
    return DropdownButtonFormField<String>(
      ...
      items: bills.map<DropdownMenuItem<String>>((FirebillModel fireBill) {
        return DropdownMenuItem<String>(
          value: fireBill.firePolicy.policyholder,
          child: Text(fireBill.firePolicy.policyholder?? '', style: const TextStyle(fontSize: 14)),
        );
      }).toList(),
    );
  }
