...
class _FireMoneyReceiptReportPageState extends State<FireMoneyReceiptReportPage> {
  late int billCount = 0;
  late double totalNetPremium = 0.0;
  late double totalTax = 0.0;
  late double totalGrossPremium = 0.0;

  List<FireMoneyReceiptModel> allMoneyReceipt = [];
  List<FireMoneyReceiptModel> filteredMoneyReceipt = [];
  ...
  double calculateTotalNetPremium() {
    return filteredMoneyReceipt.fold(0.0, (total, moneyreceipt) => total + (moneyreceipt.fireBill?.netPremium ?? 0));
  }
  ...
  double calculateTotalGrossPremium() {
    return filteredMoneyReceipt.fold(
        0.0, (total, moneyreceipt) => total + (moneyreceipt.fireBill?.grossPremium ?? 0));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Fire Money Receipt Report'),
        centerTitle: true,
